...
const METRIC_CONFIG: Record<string, any> = {
  temperature: { key: 'temperature_c_avg', rawKey: 'temperature_c', color: '#E27D60', label: 'Temperature (°C)' }, // Alert
  humidity: { key: 'humidity_pct_avg', rawKey: 'humidity_pct', color: '#8FBC8F', label: 'Humidity (%)' }, // Primary
  co2: { key: 'co2_ppm_avg', rawKey: 'co2_ppm', color: '#556B6B', label: 'CO₂ (ppm)' }, // Secondary Text
  ph: { key: 'ph_level_avg', rawKey: 'ph_level', color: '#2E8B57', label: 'pH Level' }, // Emerald
};
...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-white border border-border p-4 rounded-xl shadow-organic-md z-50">
      <p className="text-text-muted text-xs font-semibold uppercase tracking-wider mb-2">{label}</p>
...
export default function TelemetryChart({ data = [], interval = 'hour', metrics = ['temperature', 'humidity'], height = 320 }: any) {
  ...
  const chartData = data.map((point: any) => {
    ...
    const entry: any = { time: formatted };
